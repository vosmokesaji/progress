...
- 提供了这些扩展：
    - 弹出新浏览器窗口功能
    - 移动、缩放、和关闭浏览器窗口
    - 浏览器详细信息 navigator 对象
    - 浏览器所加载的页面详细信息 location 对象
    - 用户显示器详细信息的 screen 对象
    - window 对象
    - 对 cookies 的支持
...
#### 一元操作符： 只能操作一个值的操作符
- 递增 ```++``` 和递减 ```--```
    - 前置：变量的值在**语句被求值之前**改变，在计算机领域，这种情况被称为 **副效应**
    - 后置：变量的值在**语句被求值之后**改变
    - 这四个操作符可以用于 **字符串** 、 **布尔值** 、 **浮点数值** 和 **对象** ，详细规则见 Page.37
- 一元加减操作符 ```+``` 和 ```-```
    - 就是变不变号，还能转换数据类型

#### 位操作符
- 数值在内存中是 **二进制** 存储的 ， 负数使用的是 **二进制补码**
- 按位非（NOT） 用 ```~``` 表示
- 按位与（AND） 用 ```&``` 表示
- 按位或（OR）用 ```|``` 表示
- 按异或（XOR）用 ```^``` 表示
...
- 逻辑与 ```&&```
- 逻辑或 ```||```
- 逻辑与 和 逻辑或 都是**短路操作**，即如果第一个数能决定结果那么不再对第二个数求值
...
- 用于字符串的比较操作可以用来确定字母表的先后顺序，但是要注意：**要么都用大写比较，要么都用小写比较**
- 任何数与 ```NaN``` 比较，结果都是 ```false```
- 两个数字字符串的比较也会得出奇怪的结果
    ```javascript
    // 原因是都是字符串，会从左到右按位比较，左边第一个字符 "2" 编码是 30 ，右边第一个字符 "3" 编码是 51 ，
    var result = "23" < "3";    // true

    // 如果将其中一个改为数值，比较结果就正常了
