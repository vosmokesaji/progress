...
#### 1.1.5.3. 解构赋值
    ...
    console.log(z);         // "zzz"
    ```
#### 1.1.5.4. 块级作用域（ 在代码块 ```{}``` 中声明的变量，在块外访问不到 ）
    ```javascript
    // JS
    var obj = {a: 100, b: 200, c: 300};
    for(var item in obj){
...
#### 1.3.4.4. Promise.all 和 Promise.race
...
#### 1.6.5.2. 回顾 vue 修改属性也是异步的
- 效果、原因和 setState 一样
- set 中执行 updateComponent 是异步的

#### 1.6.5.3. setState 的过程
...
#### 1.6.5.4. 问题解答：说一下 React setState 的过程
- setState 是异步的： 效果、原因
- vue 修改属性也是异步的： 效果、原因
...
#### 1.7.2.1. 问题解答： 介绍一下 hybrid 更新上线流程？
- 掌握流程图
- 要点1： 服务端的版本和 zip 包维护
