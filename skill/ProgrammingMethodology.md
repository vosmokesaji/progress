...
- 变量的三个基本要素： 
    1. 名字
        - Java 中命名要以字母或下划线开头，之后可以跟字母、数字或下划线（ 不能和保留的和已知的关键字重名 ）
        - **重要的是 准确的命名是一个良好的软件工程原则**
    2. 类型
        - [int] 整型 （ integer ）-20亿 ~ +20亿
        - [double] 双精度，实数 （ real-value ）
        - [boolean] 布尔值
        - [char] 字符型  ( character )
        - 为什么区分 int 和 double 呢？
            - 有些东西需要计数，加上小数点就不合理了，需要用整数来描述数量，在实数中没有下一个值，如果你要计数，需要用整形，所以这些类型都有用
    3. 值
        - 给每个变量都赋初始值，也是个很好的软件工程习惯

> IEEE: 'I triple E' 电气与电子工程协会，所有标准都是他们制定的
- 类 -> 对象 -> 对象中包含变量
